...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="FANZA商品データを取得してHugoブログ記事を自動生成する（アニメ特化版）",
    )
    parser.add_argument(
        "--genre",
        type=str,
        default="",
        help="ジャンルキー（anime, doujin_cg, doujin_manga, doujin_voice, doujin_game, pcgame, comic, ntr_anime, tentacle, isekai, school, bl, all）",
    )
    parser.add_argument(
        "--keyword",
        type=str,
...
def run_genre(genre_key: str, count: int) -> tuple[list[dict], list[str]]:
    ...
    print(f"  service={service}, floor={floor}")
    print(f"{'='*40}")
