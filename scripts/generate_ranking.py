...
def fetch_ranking(keyword="", hits=20, service="digital", floor="anime"):
    ...
    for i, item in enumerate(items):
        ...
        affiliate_url = (
            f"https://www.dmm.co.jp/digital/anime/-/detail/=/cid={content_id}/?af_id={Config.AFFILIATE_ID}"
            if content_id
            else ""
        )
...
def generate_ranking_page(ranking_type="daily", genre_name="エロアニメ総合", genre_key="", service="digital", floor="anime"):
...
def generate_all_rankings():
    """全ジャンルのランキングを生成"""
    # エロアニメ総合ランキング
    generate_ranking_page("daily", "エロアニメ総合", "", service="digital", floor="anime")

    # ジャンル別ランキング
    genre_rankings = [
        ("NTRアニメ", "NTR 寝取られ", "digital", "anime"),
        ("触手", "触手 異種姦", "doujin", "digital_doujin"),
        ("学園", "学園 制服", "digital", "anime"),
        ("異世界", "異世界 ファンタジー", "doujin", "digital_doujin"),
    ]
    for name, keyword, service, floor in genre_rankings:
        generate_ranking_page("daily", name, keyword, service=service, floor=floor)
